...
class VAE_2D(nn.Module):
    ...
    def forward(self, x):
        ...
        # # Conv2d_output_width = [ (input_width - kernel_width + 2*padding) / stride ] + 1
        #
        # # Add all convolutional layers, with suitable padding to make kernels work
        # for i in range(numconvs):
        #     padding = 0
        #     convsize = (compressedsize + padding - kernels[i])/strides[i] + 1
        #     while (convsize != int(convsize)):
        #         padding += 1
        #         convsize = (compressedsize + padding - kernels[i])/strides[i] + 1
        #     paddinglist.append(padding)
        #     if i == 0:
        #         encoderDict['encoderConv'+str(i)] = Conv(in_channels=1, out_channels=(i+1)*base, kernel_size=(1,kernels[i]), stride=(1,strides[i]), padding=(1,padding))
        #         #print('enc', i, 1, (i+1)*base, kernels[i], strides[i])
        #     else:
        #         encoderDict['encoderConv'+str(i)] = Conv(in_channels=i*base, out_channels=(i+1)*base, kernel_size=(1,kernels[i]), stride=(1,strides[i]), padding=(1,padding))
        #         #print('enc', i, i*base, (i+1)*base, kernels[i], strides[i])
        #     compressedsize = int(convsize)
        # encoderDict['encoderFlatten'] = nn.Flatten()
        # for i in range(linear_size):
        #     if i == linear_size-1:
        #         encoderDict['encoderLinear'+str(i)] = nn.Linear(in_features=18*(numconvs+1)*base*compressedsize, out_features=latent_size)
        #     else:
        #         encoderDict['encoderLinear'+str(i)] = nn.Linear(in_features=18*(numconvs+1)*base*compressedsize, out_features=18*(numconvs+1)*base*compressedsize)
        #     encoderDict['encoderGELU'+str(i)] = nn.GELU()
        #
        # # Define encoder with suitable hyperparameters
        # self.encoder = nn.Sequential(encoderDict)
        #
        # # Add latent space parameters
        ...
        # # Add mirror image decoder network
        # # Mirror linear layers
        # for i in range(linear_size):
        #     if i == 0:
        #         decoderDict['decoderLinear'+str(i)] = nn.Linear(in_features=latent_size, out_features=18*(numconvs)*base*compressedsize)
        #     else:
        #         decoderDict['decoderLinear'+str(i)] = nn.Linear(in_features=18*(numconvs)*base*compressedsize, out_features=18*(numconvs)*base*compressedsize)
        #     decoderDict['decoderGELU'+str(i)] = nn.GELU()
        # decoderDict['decoderUnflatten'] = nn.Unflatten(dim=1, unflattened_size=[(numconvs)*base, 18, compressedsize])
        # #print('unflat size = ', [(numconvs)*base, 18, compressedsize])
        # # Mirror convolutions and take off padding
        # for i in range(numconvs):
        #     if i == numconvs-1:
        #         decoderDict['decoderConv'+str(i)] = ConvTransposeFinal(base, 1, (1,kernels[0]), stride=(1,strides[0]))
        #         #print('deconv', i, base, 1, kernels[0], strides[0])
        #     else:
        #         decoderDict['decoderConv'+str(i)] = ConvTranspose((numconvs-i)*base, (numconvs-i-1)*base, (1,kernels[-i]), stride=(1,strides[-i]))
        #         #print('deconv', i, (numconvs-i)*base, (numconvs-i-1)*base, kernels[-i], strides[-i])
