...
def prepare_VAE_MLP_joint_data(first_time_train_test_split=True, cross_val=False, fold_data=None, train_ids=None, test_ids=None, num_synthetic=10, oversample_ratio=1):
    ...
    patient_ids = []
    patients_in_cohort1 = list(set(cohort1['shortpatpseudoid']))
    for i in range(len(patient_files_list)):
        if patient_files_list[i][:10] not in patient_ids and patient_files_list[i][:10] in patients_in_cohort1:
    ...
    print('Number of single files:', len(single_file_names), single_file_names[:5])
    ...
    old_train_ids = train_ids
    old_test_ids = test_ids
    train_ids = []
    for id in old_train_ids:
        if id in patient_ids:
            train_ids.append(id)
    test_ids = []
    for id in old_test_ids:
        if id in patient_ids:
            test_ids.append(id)
